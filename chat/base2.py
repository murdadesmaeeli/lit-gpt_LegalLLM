...
def main(
    *,
    ...
    while True:
        ...
        if not prompt:
            break
        prompt,userInput = prompt.split('!!')
        prompt = system_prompt.format(prompt=prompt)
        encoded_prompt = tokenizer.encode(generate_prompt({"instruction": prompt, "input": userInput}), device=fabric.device)
        ...
        y = generate(
            model, encoded_prompt, max_output_tokens, temperature=temperature, top_k=top_k, stop_tokens=stop_tokens
        )
